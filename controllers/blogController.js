...
    if (!title || !excerpt || !content || !category || !impact) {
      return res
        .status(400)
        .json({ error: 'All required fields (title, excerpt, content, category, impact) must be filled' });
    }
...
    // Create blog entry, including the new 'impact' field
...
      .select('title slug excerpt category readingTime images createdAt views impact')
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .limit(limit);
...
    blog.impact = req.body.impact || blog.impact; // Added impact update
